...
async function main() {
  ...
  toolkit.action({
    // 工具包的名称:投资，
    //Venus协议工具包中的一个重要组成部分，专门用于处理用户的投资操作，它将用户的投资请求转换为区块链交易，并确保交易的安全性和可靠性。
    action: 'invest',
    // 描述了这个动作的功能，即在BSC链上投资资产并获得cTokens作为回报
    ...
    // 投资参数:定义了一堆参数，每个参数都有type、description、required等属性
    // 链名:BNB (a.k.a. BSC)，amount:投资金额，asset:投资资产，action:投资操作
    payloadDescription: {
  ...
  }, async (ctx: ActionContext, payload: any = {}) => {//执行逻辑，ctx是上下文，payload是参数
    try {
      //设置操作类型为'invest'
      payload.action = 'invest';
      // 调用api.createTransaction创建未签名交易
